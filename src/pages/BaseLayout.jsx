...
import { Button, Dropdown, Modal, Tab } from "bootstrap";
  ...
  useEffect(() => {
    ...
    async function fetchUserInfo() {
      ...
      if (status) {
        ...
        localStorage.setItem(
          Const.STORAGE_KEY.UPT_INFO,
          JSON.stringify(data.upt)
        );
    ...
    async function syncPresences() {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        if (localStorage.key(i).includes("__UP__")) {
          unsyncPresences.push(
            JSON.parse(localStorage.getItem(localStorage.key(i)))
          );
        }
      }
      if (unsyncPresences.length > 0) {
        const { status, data, message } = await syncPresence({
          unsyncPresences,
        });
        if (status) {
          for (let i = 0; i < localStorage.length; i++) {
    ...
    if (me == null) {
      dispatch(
        setMe(JSON.parse(localStorage.getItem(Const.STORAGE_KEY.USER_INFO)))
      );
      fetchUserInfo();
    }
  }, []);
