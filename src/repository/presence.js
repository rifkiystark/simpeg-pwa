...
const presenceStatus = async () => {
    const todayPresence = JSON.parse(localStorage.getItem(Const.STORAGE_KEY.PRESENCE))
    const unsyncPresence = JSON.parse(localStorage.getItem(Const.STORAGE_KEY.UNSYNC_PRESENCE))
...
const syncPresence = async (dataPresence) => {
    try {
        let result = await axiosIntance().post("presensi/sinkronisasi", dataPresence, {
            headers: { 'Authorization': "Bearer " + localStorage.getItem(Const.STORAGE_KEY.TOKEN) }
        });
...
export { presenceStatus, presenceIn, presenceOut, syncPresence }